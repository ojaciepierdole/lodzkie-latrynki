...
export default function CorrectionForm({ toilet, isOpen, onClose }: CorrectionFormProps) {
  ...
  // Pre-fill from toilet data when opening
  useEffect(() => {
    if (isOpen && toilet) {
      setName(toilet.name || '');
      setAddress(toilet.address || '');
      setType(toilet.type || 'free');
      setPrice(toilet.price || '');
      setAccessible(toilet.accessible || false);
      setHours(toilet.hours?.raw || '');
      setNotes('');
      setStatus('idle');
      setErrorMessage('');
    }
  ...
  }, [toilet.id, name, address, type, accessible, hours, notes, t]);
  ...
  // --- Shared inline styles (per project convention for overlay components) ---
        ...
        <div style={{ padding: '20px 20px 32px' }}>
          ...
          {status === 'success' ? (
            ...
            <>
              ...
              <p style={subtitleStyle}>{toilet.name} — {toilet.address}</p>
